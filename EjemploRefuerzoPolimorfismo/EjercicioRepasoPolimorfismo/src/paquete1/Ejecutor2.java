    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < nombres.length; i++) {
            if (edad[i] >= 0 && edad[i] <= 10) {
                ...
                pasajes.add(pMenor);
//                pMenor.establecerValorPasaje();
            }
            if (edad[i] > 18 && edad[i] <= 25) {
                Persona p = new Persona(nombres[i], apellidos[i], edad[i]);

                PasajeUniversitario pUni = new PasajeUniversitario(pasajeFijo);
                pUni.establecerPersona(p);
                pasajes.add(pUni);

//                pUni.establecerValorPasaje();
            }
...
//                pNormal.establecerValorPasaje();
            }
            if (edad[i] >= 65) {
                Persona p = new Persona(nombres[i], apellidos[i], edad[i]);

                PasajeTerceraEdad pTercera = new PasajeTerceraEdad(pasajeFijo);
                pTercera.establecerPersona(p);
                pasajes.add(pTercera);

//                pTercera.establecerValorPasaje();
            }
        }
